  ...
  return (
    <div className="min-h-screen bg-deep-green text-foreground py-8 px-4">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold mb-8">매칭 결과 관리</h1>
        ...
        <div className="bg-primary/20 rounded-lg p-6 mb-6">
          <h2 className="text-2xl font-semibold mb-4">참가자별 선택 결과</h2>
          <div className="space-y-4">
            {likes.map((like) => (
              <div key={like.uid} className="bg-deep-green rounded-lg p-4">
        ...
        {/* 매칭 결과 */}
        <div className="bg-primary/20 rounded-lg p-6 mb-6">
          <h2 className="text-2xl font-semibold mb-4">매칭 결과</h2>
          <div className="space-y-4">
            {matches.length === 0 ? (
              <p className="text-gray-300">매칭 결과가 없습니다.</p>
            ) : (
              matches.map((match, index) => (
                <div key={index} className="bg-deep-green rounded-lg p-4">
                  <p className="font-semibold mb-2">
                    커플 {index + 1}: {match.userA} ↔ {match.userB}
                  </p>
                  <p className="text-sm text-gray-300">매칭 점수: {match.score}</p>
                </div>
              ))
            )}
          </div>
        ...
        <div className="bg-primary/20 rounded-lg p-6">
          <h2 className="text-2xl font-semibold mb-4">인기 순위 (Top 3)</h2>
          <div className="space-y-4">
            {/* 실제로는 likes 데이터를 집계해서 계산해야 함 */}
            <p className="text-gray-300">인기 순위 계산 기능 구현 필요</p>
          </div>
        </div>
      </div>
    </div>
